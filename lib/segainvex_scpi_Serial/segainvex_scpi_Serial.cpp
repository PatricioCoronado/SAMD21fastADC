/*************************************************************************************
	...
	LIBRERIA PARA ARDUINO segainvex_scpi_Serial SERIAL V1.0
...
/* 
		...
		Para seleccionar el puerto, la librería hace una compilación
...
**************************************************************************************/
#include <Arduino.h>
#include "segainvex_scpi_Serial.h"//Declaraciones privadas de segainvex_scpi_Serial
/************************************************************************************
...
char IdentificacionDelSistema[64]="Identificacion del sistema";//Cadena con el nombre del sistema
/***************************************************************************************
...
void errorscpi(int codigo) {
  ...
  if (codigo == 0) //Si el parámetro es 0..
	...
	#if defined(SERIALUSB)  
		SerialUSB.println(CodigoError[errores[numero_errores]]);
	#elif defined(SERIAL1)  
		Serial1.println(CodigoError[errores[numero_errores]]);
	#else  
	...
	#if defined(SERIALUSB)  
		SerialUSB.println("0 No hay errores");
	#elif defined(SERIAL1)  
		Serial1.println("0 No hay errores");
	#else  
...
/**************************************************************************************************
			...
			segainvex_scpi_Serial busca cadenas válidas que podrían ser comandos en la cadena de entrada.
...
void scpi(void)
{
	...
	#if defined(SERIALUSB)  
		locCom=SerialUSB.readBytesUntil('\r',buffCom, BUFFCOM_SIZE); 
	#elif defined(SERIAL1)  
		locCom=Serial1.readBytesUntil('\r',buffCom, BUFFCOM_SIZE); 
		//SerialUSB.println("Estoy en Serial1");
	#else  
		locCom=Serial.readBytesUntil('\r',buffCom, BUFFCOM_SIZE); 
		//SerialUSB.println("Estoy en Serial a secas");
	#endif  
...
/*********************************************************************************************
						FUNCIONES segainvex_scpi_Serial COMUNES A TODOS LOS SISTEMAS 
**********************************************************************************************/
/**********************************************************************
    Comando: *IDN"
    Función:240
...
{
	#if defined(SERIALUSB)  
		SerialUSB.println(IdentificacionDelSistema);
	#elif defined(SERIAL1)  
		Serial1.println(IdentificacionDelSistema);
	#else  
 ...
 *********************************************************************/
int fs243(void){if (FinComando[0] == '?')errorscpi(0);
	else errorscpi(4);return 0;}
 /**********************************************************************
  Comando:*OPC
  Función:248
 ...
 *********************************************************************/
int fs255(void){errorscpi(-1);return 0;}
/*********************************************************************/
...
int cambia_variable_double_del_sistema(double *pVariableDelSistema,double Maximo, double Minimo)
{
  ...
  {
	#if defined(SERIALUSB)  
		SerialUSB.println(*pVariableDelSistema);return 2;
	#elif defined(SERIAL1)  
		Serial1.println(*pVariableDelSistema);return 2;
	#else  
